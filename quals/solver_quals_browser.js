// browser_quals.js - Browser-based qualification tests for Reciplier
//
// This file is injected into the page by quals/run_quals.js
// It can also be loaded manually in the browser console for debugging.
//
// Usage: runQuals()
...
function runQuals() {
  ...
  // Pythagorean-style: expect sqrt(2)
  ;(() => {
    const eqns = [
      ['x', 1],
      ['A', 'a*x'],
  ...
  // Cheesepan-style constraint chain
  ...
  // Regression: interactive-style re-solve
  ;(() => {
    const eqns = [
      ['x/2', 0.5],
    ]
  ...
  // Regression: definition pairs should propagate directionally
  ;(() => {
    const eqns = [
      ['d', 66],
      ['w', 5.45],
  ...
  // Bug 1b: Same dial recipe bug but with exact repro steps
  ;(() => {
  ...
  // Bug 2: Breakaway biscuits - typing k*d
  ;(() => {
    const k = 0.621371
    const eqns = [
      ['k', k],
