...
**Solver relaxation drift (RESOLVED)**: editing a field used to perturb unrelated seeded values (dumbdial: tfin=60 gave r=5.1018, vfin=306.11 instead of r=5, vfin=300; biketour: editing break 3 moved b1/b2/start-time). Root cause: the edited variable's STALE seed contradicts the edit's own equation, and the relaxation ladder dropped innocent seeds before the conflicting one. Fix in solvem: seeds of vars in equations violated at the seed point ("suspects") drop first, with directional propagation -- a suspect inside a compound term implicates the equation's bare-var terms (in {b = b1+b2+b3}, changed b3 implicates b, not b1/b2). Both cases now change minimally and the biketour/dumbdial reciplate quals pin the minimal-change outcomes. Note: flipping the fewest-appearances tiebreak to most-first was tried and rejected -- it breaks 6 conflicting-seed quals.
