...
## Codebuff's Anti-Magic / Anti-Postel Improvement Checklist

### Predict-vs-Verify Violations (High Priority)
- ✅ **gaussJordan hasUnknowns check** (csolver.js ~line 367): Removed predict-bail pattern that checked for unknowns before attempting solve
- ✅ **newtRaphson potentiallyUnderdetermined check** (csolver.js ~line 1165): Removed predict-bail based on equation/variable count
- ✅ **newtRaphson zero-value bailout** (csolver.js ~line 1254): Replaced bailout with perturbation to break symmetry
- ✅ **knownVars complexity**: Removed entire knownVars parameter/concept - seeds are just seeds, constraints are equations
...
Based on AGENTS.md principles:

1. **Anti-Robustness audit**: Search for silent error handling that should be loud failures
2. **Naming audit**: Review symbol names for greppability, pronounceability, jargon quality
3. **Comment preservation**: Verify no human-written comments were modified during previous changes
4. **Cyclomatic complexity**: Identify remaining high-complexity functions that could be simplified
